...
func GetAllUsers(w http.ResponseWriter, r *http.Request) {
	...
	for _, recommendedUser := range apiResponse.RecommendedUsers {
		user, err := pClient.Client.User.FindUnique(
			db.User.ID.Equals(recommendedUser.UserID),
		).Exec(pClient.Context)
		if err != nil {
			http.Error(w, "Cannot fetch details for user "+recommendedUser.UserID, http.StatusInternalServerError)
			return
		}
		allUserDetails = append(allUserDetails, struct {
			UserID   string `json:"userId"`
			Name     string `json:"name"`
			Avatar   string `json:"avatar"`
			Username string `json:"username"`
	...
	responseData, err = json.Marshal(allUserDetails)
	if err != nil {
